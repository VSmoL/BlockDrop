// ArrayPrefs2 v 1.3
// todo: modify PlayerPrefx.js as EditorPrefs.cs has been modified, to allow 0-length arrays.
...
static function GetBoolArray (key : String) : boolean[] {
	if (PlayerPrefs.HasKey(key)) {
		...
		if (bytes[0] != EditorArrayType.Bool) {
...
static function GetStringArray (key : String) : String[] {
	if (PlayerPrefs.HasKey(key)) {
		...
		if (bytes[0] != EditorArrayType.String) {
...
private static function SetValue (key : String, array : IList, arrayType : EditorArrayType, vectorNumber : int, convert : function(IList, byte[], int)) : boolean {
	if (array.Count == 0) {
...
private static function GetValue (key : String, list : IList, arrayType : EditorArrayType, vectorNumber : int, convert : function(IList, byte[])) {
	if (PlayerPrefs.HasKey(key)) {
		var bytes = System.Convert.FromBase64String (PlayerPrefs.GetString(key));
...
static function ShowArrayType (key : String) {
	var bytes = System.Convert.FromBase64String (PlayerPrefs.GetString(key));
	if (bytes.Length > 0) {
		var arrayType : EditorArrayType = bytes[0];
